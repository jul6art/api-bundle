# Contributing to `jul6art/api-bundle`

`jul6art/api-bundle` is a set of API Platform filters and state providers extracted from a
working application: human ordering of text columns, ordering by business rank rather than
alphabet, one sortable property spanning several columns, one search box over several
columns, filtering by year, searching inside a JSON column, a custom provider that keeps
pagination, filters and ordering, and a tenant header.
...
* **Bug** → open an [issue](https://github.com/jul6art/api-bundle/issues/new/choose)
...
```bash
git clone https://github.com/jul6art/api-bundle.git
cd api-bundle
composer install
```
...
`jul6art/api-bundle` is installed by other projects through Composer, so the version number
...
2. **Every value that reaches the query builder is bound, never interpolated.** These
   filters exist to turn request input into DQL, which makes them the one place in the
   ecosystem where a string concatenation is a vulnerability rather than a style problem.
   Property names are validated against what the resource declares; values go through
   parameters.

3. **A filter only ever touches a property the resource exposes.** A filter that can be
   pointed at an unmapped or unexposed field turns an ordering feature into a read
   primitive.

4. **A state provider preserves what API Platform gave it** — the operation's security
   expression, the pagination bounds, the filters and the ordering. A provider that returns
   an unbounded collection is a denial of service; one that drops the security expression is
   worse.

5. **Pagination stays bounded.** No change may allow a client to ask for an unlimited page
   size.

6. **Interface strings are translation keys, translated in both `en` and `fr`**
